...
def register_admin(request):
    if request.method == "POST":
        ...
        contact_number = request.POST.get('contact_number')
        store_id = request.POST.get('stall')

        try:
            stall = Stall.objects.get(store_id=store_id)
            ...
            try:
                ...
                messages.success(request, f'Admin {firstname} {lastname} has been successfully registered!')
    ...
    # Get only stalls without admins
    assigned_stall_ids = AdminProfile.objects.values_list('stall__store_id', flat=True)
    available_stalls = Stall.objects.exclude(store_id__in=assigned_stall_ids)
    admins = AdminProfile.objects.all()
    
    return render(request, 'TriadApp/superadmin/register_admin.html', {
        'stalls': available_stalls,  # Only show available stalls
        'admins': admins
    })
